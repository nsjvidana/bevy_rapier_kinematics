...
pub fn set_ik_arm_positions(
    ...
    for (arm_info, ik_arm) in ik_arm_q.iter_mut() {
        ...
        if wrist_pos.distance(target_pos.into()) <= ik_arm.ik_solver.allowable_target_distance {
            continue;
        }


        if let Some(ik_pole) = &ik_arm.elbow_ik_pole {
            let shoulder_pos = Vector3::<Real>::from(
                upper_transform.mul(Vec3::new(0., upper_arm_len/2., 0.))
            );
            let mut dir_to_pole = ik_pole - Vector3::<Real>::from(shoulder_pos);
                dir_to_pole.normalize_mut();
            for node in ik_arm.chain.iter() {
                let name = &node.joint().name;
                let node_pos =
                    if name.starts_with("shoulder") {
                        shoulder_pos
                    }
                    else if name.starts_with("elbow") {
                        dir_to_pole.scale(upper_arm_len)
                    }
                    else {
                        dir_to_pole.scale(upper_arm_len + lower_arm_len)
                    };
